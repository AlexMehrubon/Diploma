...
watch(searchText, async (text) => {
  await coursesStore.fetch(1,8,{ "filter[name]": text});
})

</script>

<template>
  <p class="text-xl mb-2 font-bold">Управление курсами</p>
  <div class="pb-4 bg-white dark:bg-gray-900 flex flex-row justify-between">
    ...
    <button
      @click="coursesStore.switchCreateModal"
  ...
  <div v-if="coursesStore.isLoading" class="flex flex-col justify-center items-center h-110 text-lg">
    <div role="status">
  ...
  <div v-else class="flex flex-col h-full pb-5 justify-between">
    <CourseTable :courses-store="coursesStore"/>
    <Paginator :store="coursesStore"/>
  </div>
</template>
