...
import ModuleCreateModal from "@/components/modules/ModuleCreateModal.vue";
...
const initCourseData = () => {
  const currentCourse = props.coursesStore.currentEditCourse;
  course.value = {
    id: currentCourse.id,
    ...
    difficulty_level_id: currentCourse.difficulty_level_id,
    status_id: currentCourse.status_id,
    about: currentCourse.about,
    average_salary: currentCourse.average_salary,
    tags: currentCourse.tags.map(tag => tag.id) // Извлекаем только ID тегов
  };
};
...
onMounted(async () => {
  initCourseData();
  const filter = {
    "filter[course_id]": props.coursesStore.currentEditCourse.id
  };
  moduleStore.currentCourse = props.coursesStore.currentEditCourse.id;
  await moduleStore.fetch(1, 8, filter, false);
});

watch(() => props.coursesStore.currentEditCourse, () => {
  initCourseData();
  const filter = {
    "filter[course_id]": props.coursesStore.currentEditCourse.id
  };
  moduleStore.fetch(1, 8, filter, false);
});
</script>

<template>
  <ModuleCreateModal
    v-if="moduleStore.isShowedCreateModal"
    :courses-store="props.coursesStore"
    :module-store="moduleStore"
  />
    ...
    <div class="relative w-full max-w-4xl p-4 mx-auto">
      ...
      <div class="relative bg-white rounded-xl shadow-2xl overflow-hidden">
        ...
        <div class="flex items-center justify-between p-6 border-b border-gray-100 bg-gradient-to-r from-blue-50 to-indigo-50">
          ...
          <button
            @click="props.coursesStore.switchEditModal"
            type="button"
          ...
          <form class="space-y-6">
            ...
            <div>
              <div class="flex flex-row justify-between items-center py-2">
                <label class="block text-sm font-medium text-gray-700">Модули курса</label>
                <button
                  @click="moduleStore.switchCreateModal"
                  type="button"
                  class="text-white bg-green-600 hover:bg-green-700 focus:ring-2 focus:ring-green-500 font-medium rounded-lg text-sm px-4 py-2.5 transition-all duration-200"
                >
                  Добавить модуль
                </button>
              </div>
              ...
              <ModuleTable
                :course-store="props.coursesStore"
                :module-store="moduleStore"
              />
            </div>
            ...
            <div class="pt-4">
              <button
                @click.prevent="props.coursesStore.updateCourse(course.id, course)"
                v-if="!props.coursesStore.isUpdating"
